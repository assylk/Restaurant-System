...
class PDF extends FPDF
{
    function Header()
    {
        ...
        $this->Cell(0, 10, "Taste.it Dining & Bar Report", 0, 1, 'C');
...
$pdf->ChapterBody("Total Revenue Today: DT " . number_format($totalRevenueToday, 2));
$pdf->Ln();
...
$pdf->ChapterBody("Total Revenue This Week: DT " . number_format($totalRevenueThisWeek, 2));
$pdf->Ln();
...
$pdf->ChapterBody("Total Revenue This Month: DT " . number_format($totalRevenueThisMonth, 2));
$pdf->Ln();
...
$pdf->ChapterBody("Total Revenue This Year: DT " . number_format($totalRevenueThisYear, 2));
$pdf->Ln();
...
$header = array('Date','Day' , 'Revenue (DT)');
$data = array();
...
$header = array('Date','Week' , 'Revenue (DT)');
$data = array();
...
$header = array('Date','Month' , 'Revenue (DT)');
$data = array();
...
$header = array('Date','Day' , 'Item Category', 'Revenue (DT)');
$data = array();
while ($row = mysqli_fetch_assoc($dailyCategoryRevenue)) {
...
$header = array('Date','Week' , 'Item Category', 'Revenue (DT)');
$data = array();
while ($row = mysqli_fetch_assoc($weeklyCategoryRevenue)) {
...
$header = array('Date','Month' , 'Item Category', 'Revenue (DT)');
$data = array();
while ($row = mysqli_fetch_assoc($monthlyCategoryRevenue)) {
...
$header = array('Date', 'Item Category', 'Revenue (DT)');
$data = array();
